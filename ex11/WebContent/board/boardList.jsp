		...
		<div class = "cls2">
			<c:if test="${totalCnt != null}">
				<c:choose>
						...
						<c:if test="${(section*100) < totalCnt}"><!-- >> -->
							<c:forEach var="page" begin="1" end="10" step="1">
								<c:if test="${section >1 && page==1}">
									<a href="BS?command=board_list&section=${section-1}&pageNum=${10}"> pre </a>
								</c:if>
									<a href="BS?command=board_list&section=${section}&pageNum=${page}">${(section-1)*10+page}</a>
								<c:if test="${page == 10}">
									<a href="BS?command=board_list&section=${section+1}&pageNum=${1}"> next </a>
								</c:if>
							</c:forEach>
						</c:if>
						<c:if test="${(section)*100 > totalCnt}">
					...
					<c:when test="${totalCnt < 100}">
						<c:forEach var="page" begin="1" end="${(totalCont+9)/10}" step="1">
							<a href="BS?command=board_list&section=${section}&pageNum=${page}">${page}</a>
						</c:forEach>
					</c:when>
				</c:choose>
